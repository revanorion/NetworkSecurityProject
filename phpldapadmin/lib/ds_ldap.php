	...
	private function unescapeDN($dn) {
		...
		if (is_array($dn)) {
			...
			foreach ($dn as $key => $rdn)
				...
				$a[$key] = preg_replace_callback('/\\\([0-9A-Fa-f]{2})/e',function ($matches) { return chr(hexdec($matches[1])); }, $rdn);

			return $a;

		} else
			#return preg_replace('/\\\([0-9A-Fa-f]{2})/e',"''.chr(hexdec('\\1')).''",$dn);
			return preg_replace_callback('/\\\([0-9A-Fa-f]{2})/e',function ($matches) { return chr(hexdec($matches[1])); }, $dn);
	}
